  ...
  try {
    ...
    const newPost = await databases.createDocument(
      ...
      ID.unique(),
      {
        ...
        imageUrl: fileUrl, // agora é uma URL válida
...
export async function uploadFile(file: File): Promise<{ $id: string } | undefined> {
  try {
...
export async function getUsers(limit?: number) {
    ...
    try {
        const users = await databases.listDocuments(appwriteConfig.databaseId, appwriteConfig.userColletionId, queries)
...
export async function getUserById(userId: string) {
    try {
        const user = await databases.getDocument(appwriteConfig.databaseId, appwriteConfig.userColletionId, userId)
...
export async function updateUser(user: IUpdateUser) {
    ...
    try {
        ...
        //  Update user
        const updatedUser = await databases.updateDocument(appwriteConfig.databaseId, appwriteConfig.userColletionId, user.userId, {
            name: user.name,
            bio: user.bio,
            imageUrl: image.imageUrl,
            imageId: image.imageId,
